...
describe('_validSupport', () => {
  ...
  it('returns true (false) for a correct (incorrect) set of signatures on n states', async () => {
    ...
    expect(await ForceMove.validSupport(8, addresses, stateHashes, sigs, whoSignedWhat)).toBe(true);
    const brokenSigs = sigs.reverse();
    expect(await ForceMove.validSupport(8, addresses, stateHashes, brokenSigs, whoSignedWhat)).toBe(
      false,
    );
  });
  it('returns true (false) for a correct (incorrect) set of signatures on 1 state', async () => {
    ...
    expect(await ForceMove.validSupport(8, addresses, [stateHash], sigs, whoSignedWhat)).toBe(true);
    const brokenSigs = sigs.reverse();
    expect(await ForceMove.validSupport(8, addresses, [stateHash], brokenSigs, whoSignedWhat)).toBe(
      false,
    );
  });
});
