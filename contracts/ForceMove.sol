...
contract ForceMove {
    ...
    function forceMove(
        ...
        require(
            _validSupport(largestTurnNum, fixedPart.participants, stateHashes, sigs, whoSignedWhat),
            'Invalid signatures'
        );
    ...
    function _acceptableWhoSignedWhat(
        ...
        require(
            ...
            '_validSupport: whoSignedWhat must be the same length as participants'
        );
        for (uint256 i = 0; i < nParticipants; i++) {
    ...
    function _conclude(
        ...
        // check the supplied states are supported by n signatures
        require(
            _validSupport(largestTurnNum, participants, stateHashes, sigs, whoSignedWhat),
            'Invalid signatures'
        );
