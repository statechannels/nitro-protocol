...
library Commitment {
    ...
    function isPreFundSetup(CommitmentStruct memory self) public pure returns (bool) {
        return self.commitmentType == uint256(CommitmentType.PreFundSetup);
    }

    function isPostFundSetup(CommitmentStruct memory self) public pure returns (bool) {
        return self.commitmentType == uint256(CommitmentType.PostFundSetup);
    }

    function isApp(CommitmentStruct memory self) public pure returns (bool) {
        return self.commitmentType == uint256(CommitmentType.App);
    }

    function isConclude(CommitmentStruct memory self) public pure returns (bool) {
        return self.commitmentType == uint256(CommitmentType.Conclude);
    }
    ...
    function requireSignature(CommitmentStruct memory _commitment, uint8 _v, bytes32 _r, bytes32 _s)
        public
        pure
    {
        require(
    ...
    function requireFullySigned(
        CommitmentStruct memory _commitment,
        uint8[] memory _v,
        bytes32[] memory _r,
        bytes32[] memory _s
    ) public pure {
        for (uint256 i = 0; i < _commitment.participants.length; i++) {
            require(
                _commitment.participants[i] ==
                    recoverSigner(abi.encode(_commitment), _v[i], _r[i], _s[i]),
                "all movers must have signed Commitment"
            );
        }
    }

    function appAttributesEqual(
        CommitmentStruct memory _commitment,
